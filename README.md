...
Note: the rolling Z-score (`zscore_7`) is also one of the four features fed to Isolation Forest and LOF, so the three votes aren't fully independent — the Z-score baseline informs the ML models too, not just its own standalone vote. The Z-score baseline itself only looks at *prior* transactions for a customer (never the current row), so a large transaction can't inflate its own baseline and dampen its own score.
